...
def get_correlated_stocks(code, name, key_hash, is_us=False): 
    ...
    for attempt in range(2): 
        try: 
            ...
            prompt = f"針對 {market} {name}({code})，找出 3 檔同產業高連動的股票。嚴格規定：只能輸出代碼。{rule_str}。" 
...
def render_mini_chart(df_1m, cdp_nh, cdp_nl, alerts=[], is_us=False): 
    ...
    base_vol = alt.Chart(chart_df).encode(x=alt.X('x_idx:Q', title='', scale=alt.Scale(domain=[0, end_idx]), axis=alt.Axis(labels=False, ticks=False))) 
    ...
    if not st.session_state.authenticated: 
        ...
        if st.button("解鎖戰情室", use_container_width=True, type="primary"): 
            ...
            if totp.verify(auth_code): 
                ...
                st.success("✅ 身分驗證成功，武器系統已解鎖！"); st.rerun() 
            else: st.error("❌ 驗證碼錯誤") 
    else: 
...
if twii_live_p and twii_mas: 
    ...
    for idx, (ma_name, ma_val) in enumerate(twii_mas.items()): 
        dist_pts = twii_live_p - ma_val; dist_pct = dist_pts / ma_val 
...
tab_tw, tab_us, tab_ai, tab_core, tab_radar, tab_ptt = st.tabs(["🇹🇼 台股極速當沖", "🇺🇸 美股波段戰情", "🤖 AI 選股報告", "🐢 10年期核心長線", "📡 爆量雷達快篩", "💬 PTT 自動登入"]) 
...
with tab_tw: 
    ...
    for idx, stock in enumerate(st.session_state.tw_stocks): 
                ...
                if cond == ">=" and curr_p >= t_p: 
                    ...
                    if not al['triggered']: 
                        ...
                        if stock.get('auto_trade', False) and st.session_state.authenticated: 
                            ...
                            if res.get('status') == 'success': send_telegram_alert(f"🚀 【自動下單成功】\n{name}({code}) 漲破 {t_p_label}\n已自動送出 {act.upper()} 委託！") 
                            else: send_telegram_alert(f"❌ 【自動下單異常】\n{name}({code}) 觸發到價但送單失敗: {res.get('msg')}") 
                    ...
                    if not al['triggered']: 
                        ...
                        if stock.get('auto_trade', False) and st.session_state.authenticated: 
                            act = "buy" if stock.get('my_dir', '作多') == "作多" else "sell" 
                            res = fire_order_to_agent(code, float(stock.get('my_price', curr_p)), act, int(stock.get('my_lots', 1))) 
                            if res.get('status') == 'success': send_telegram_alert(f"🚀 【自動下單成功】\n{name}({code}) 跌破 {t_p_label}\n已自動送出 {act.upper()} 委託！") 
                            else: send_telegram_alert(f"❌ 【自動下單異常】\n{name}({code}) 觸發到價但送單失敗: {res.get('msg')}") 
                 
                touches = 0 
                if len(df_1m) >= 15: 
                    if cond == ">=": touches = (df_1m['High'].tail(15) >= t_p).sum(); touches = max(touches, 1) if curr_p >= t_p else touches 
                    else: touches = (df_1m['Low'].tail(15) <= t_p).sum(); touches = max(touches, 1) if curr_p <= t_p else touches 
        ...
        with st.container(border=True): 
            ...
            with c_del:  
                st.button("❌", key=f"del_tw_{code}", on_click=cb_remove_tw, args=(idx,)) 
                ...
                if len(df_today) > 5: 
                    ...
                    if not spikes.empty: 
                        ...
                        for _, row in spikes.iterrows(): 
                            t_str = row['Time'].strftime("%H:%M"); is_buy = row['Close'] >= row['Open'] 
                            icon = "🔴" if is_buy else "🟢"; bg_color = "rgba(239, 68, 68, 0.15)" if is_buy else "rgba(16, 185, 129, 0.15)" 
                            border_color = "#ef4444" if is_buy else "#10b981"; text_color = "#f87171" if is_buy else "#34d399" 
...
with tab_us: 
    ...
    for idx, stock in enumerate(st.session_state.us_stocks): 
        ...
        with st.container(border=True): 
            ...
            with c_s1: st.metric("支撐(S1)", f"${s1:.2f}", delta_color="off") 
            with c_del:  
                st.button("❌", key=f"del_us_{code}", on_click=cb_remove_us, args=(idx,)) 
            ...
            if not df_1m_us.empty: 
                ...
                if len(df_today) > 5: 
                    ...
                    if not spikes.empty: 
                        ...
                        for _, row in spikes.iterrows(): 
                            t_str = row['Time'].strftime("%H:%M"); is_buy = row['Close'] >= row['Open'] 
                            icon = "🟢" if is_buy else "🔴"; bg_color = "rgba(16, 185, 129, 0.15)" if is_buy else "rgba(239, 68, 68, 0.15)" 
                            border_color = "#10b981" if is_buy else "#ef4444"; text_color = "#34d399" if is_buy else "#f87171" 
                    ...
                    with sub_tabs[i]: 
                        for s in stocks: 
                            ...
                            with st.expander(f"🎯 {s['name']}({s['code']}) | 真實現價: {c_p or '--'} | 建議目標價: {target}"): 
                                ...
                                if market_type == "美股":  
                                    st.button(btn_txt, key=f"btn_u_{s['code']}_{target}_{cat}", on_click=cb_add_us, args=(s['code'], s['name'], target, cond)) 
                                else:  
                                    st.button(btn_txt, key=f"btn_t_{s['code']}_{target}_{cat}", on_click=cb_add_tw, args=(s['code'], s['name'], target, cond)) 
                ...
                if found_targets: 
                    ...
                    for t in found_targets: 
                        with st.container(border=True): 
                            ...
                            c3.button("➕ 加入監控", key=f"radar_add_{t['code']}", on_click=cb_add_tw, args=(t['code'], t['code'])) 
                else: st.info("掃描完畢。目前的目標池中尚未發現明顯的 5K/15K 爆量跡象。") 
 
# ==================== 
# 戰區 6：💬 PTT 自動登入與次數累積管理 (完全獨立) 
# ==================== 
with tab_ptt: 
    st.markdown("### 💬 PTT 自動登入與次數累積中心") 
    st.caption("💡 此功能與股市監控完全隔離。您可在此安全填寫 PTT 帳號密碼，系統會提供加密/本地記憶，並完整保留每天的登入成功與失敗日誌記錄。") 
     
    ptt_config = load_ptt_config() 
     
    with st.form("ptt_credentials_form"): 
        st.markdown("##### 🔐 PTT 安全憑證設定") 
        col_u, col_p = st.columns(2) 
        with col_u: 
            p_user = st.text_input("PTT 帳號 (ID)", value=ptt_config.get("username", "")) 
        with col_p: 
            p_pass = st.text_input("PTT 密碼", type="password", value=ptt_config.get("password", "")) 
         
        p_auto = st.checkbox("啟用每日背景登入排程標記", value=ptt_config.get("auto_enabled", False)) 
         
        submitted = st.form_submit_button("💾 儲存設定並立即執行登入測試", type="primary") 
        if submitted: 
            save_ptt_config(p_user, p_pass, p_auto) 
            st.success("✅ PTT 憑證已安全儲存至本地端！") 
             
            with st.spinner("正在連線至 ptt.cc 模擬互動登入中..."): 
                success, msg = execute_ptt_login(p_user, p_pass) 
                 
            history = load_ptt_history() 
            now_t = datetime.datetime.now(pytz.timezone('Asia/Taipei')).strftime("%Y-%m-%d %H:%M:%S") 
            status_str = "成功" if success else "失敗" 
            history.insert(0, {"時間": now_t, "帳號": p_user, "狀態": status_str, "詳細訊息": msg}) 
            save_ptt_history(history) 
             
            if success: 
                st.success(f"🎉 PTT 登入測試成功！{msg}") 
            else: 
                st.error(f"❌ PTT 登入測試失敗：{msg}") 
            st.rerun() 
 
    st.divider() 
    st.markdown("##### 📊 PTT 歷史登入稽核與次數追蹤紀錄") 
    history = load_ptt_history() 
    if history: 
        df_history = pd.DataFrame(history) 
        st.dataframe(df_history, use_container_width=True) 
        if st.button("🗑️ 清空歷史紀錄", key="clear_ptt_history"): 
            save_ptt_history([]) 
            st.success("已清空歷史紀錄。") 
            st.rerun() 
    else: 
        st.info("目前尚無 PTT 登入歷史紀錄。請於上方填寫帳號密碼並點擊儲存執行測試。") 
