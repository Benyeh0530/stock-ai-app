...
def get_kline_data(code, suffix, interval, time_key):
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        # 放大 range 以支援更長的 K 線追溯 (5m -> 60d, 15m -> 60d)
    ...
    for attempt in range(2):
        try:
            ...
            res = ai_model.generate_content(
                prompt, 
                generation_config=genai.types.GenerationConfig(temperature=0.1), # 調低溫度提高穩定性
...
def render_mini_chart(df_1m, cdp_nh, cdp_nl, alerts=[], is_us=False):
    ...
    df_melted = chart_df.melt(['Time', 'Open', 'Volume'], var_name='線型', value_name='價格')
    ...
    base = alt.Chart(df_melted).encode(
        x=alt.X('Time:T', title='', scale=alt.Scale(domain=[start_time.isoformat(), end_time.isoformat()]), axis=alt.Axis(format='%H:%M', grid=False, tickCount=10))
    )
    ...
    for al in alerts:
        if al.get('type') == '固定價格' and al.get('price', 0) > 0:
            ...
            rule = alt.Chart(pd.DataFrame({'價格': [alert_price]})).mark_rule(color='#eab308', strokeWidth=2, strokeDash=[4, 4]).encode(y='價格:Q')
    ...
    for idx, stock in enumerate(st.session_state.tw_stocks):
        ...
        if not df_1m.empty and not df_daily.empty:
            ...
            if not df_daily_rt.empty:
                ...
                mas['日線3MA'] = df_daily_rt['Close'].tail(3).mean()
                mas['日線5MA'] = df_daily_rt['Close'].tail(5).mean()
                mas['日線10MA'] = df_daily_rt['Close'].tail(10).mean()
                mas['日線23MA'] = df_daily_rt['Close'].tail(23).mean()
                mas['日線90MA'] = df_daily_rt['Close'].tail(90).mean()
            ...
            with st.container(border=True):
                ...
                with c_pnl:
                    if has_pos:
                        if st.session_state.authenticated:
                            ...
                            st.metric("未實現淨利", "*** (鎖定)", "登入後查看", delta_color="off")
                    else:
                        st.metric("未實現淨利", "--", delta_color="off")
                ...
                if not corr_codes:
                    ...
                    st.markdown("<div style='font-size:0.9rem; color:#94a3b8; margin-top:-10px; margin-bottom:10px;'>🔗 <b>族群聯動雷達：</b> 網路擁塞，AI 重新鎖定中... (請稍候重試)</div>", unsafe_allow_html=True)
                else:
                    corr_display = []
                ...
                with c_chart1:
                    st.caption("📉 **極速十字走勢圖** (含黃虛線警示防線與量能)")
                ...
                if st.session_state.authenticated:
                    ...
                    with c_order1:
                        st.markdown(f"⚡ **雲端中控：火力打擊區** (張數設定: {my_l})")
                    with c_order2:
                        if st.button(f"🔴 閃電買進", key=f"fire_b_tw_{code}", use_container_width=True, type="primary"):
                            ...
                            if res.get('status') == 'success':
                                st.toast(f"✅ 地端 Agent 收到買進指令: {code}", icon='🔥')
                            else:
                    ...
                    with c_order3:
                        if st.button(f"🟢 閃電賣出", key=f"fire_s_tw_{code}", use_container_width=True):
                            ...
                            if res.get('status') == 'success':
                                st.toast(f"✅ 地端 Agent 收到賣出指令: {code}", icon='❄️')
                            else:
                                st.error(f"❌ {res.get('msg')}")
                else:
                ...
                with st.expander("⚙️ 展開設定：持倉參數 & 專屬監控防線", expanded=False):
                    if st.session_state.authenticated:
                        ...
                        st.warning("🔒 請先通過 2FA 雙因子認證才能修改機密部位參數。")
        ...
        if live_cp is not None and not df_daily.empty:
            ...
            with st.container(border=True):
                ...
                with c_pnl:
                    if has_pos_us:
                        if st.session_state.authenticated:
                            ...
                            st.metric("未實現損益", "*** (鎖定)", "登入後查看", delta_color="off")
                    else:
                        st.metric("未實現損益", "--", delta_color="off")
                ...
                if st.session_state.authenticated:
                    c_order1, c_order2, c_order3 = st.columns([2, 1, 1])
                    with c_order1:
                        st.markdown(f"⚡ **雲端中控：火力打擊區** (股數設定: {my_l_us})")
                    with c_order2:
