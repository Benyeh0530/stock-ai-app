...
if auto_refresh: time.sleep(3); st.rerun()import streamlit as st
import pandas as pd
import google.generativeai as genai
import datetime
import requests
        ...
        if not df_1m.empty and not df_daily.empty:
            ...
            with st.container(border=True):
                ...
                if not corr_codes:
                    ...
                    st.markdown("<div style='font-size:0.9rem; color:#94a3b8; margin-top:-10px; margin-bottom:10px;'>🔗 <b>族群聯動雷達：</b> 網路擁塞，AI 重新鎖定中... (請稍候自動重試)</div>", unsafe_allow_html=True)
                else:
                    corr_display = []
                    for i, c in enumerate(corr_codes):
